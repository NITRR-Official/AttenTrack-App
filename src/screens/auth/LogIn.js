...
const LogIn = () => {
  ...
  const handleTeacherLogin = async () => {
    try {
      ...
      const response = await axios.post('https://attentrackbackend-production.up.railway.app/api/teacher/login', {
        email: email,
        password: password,
      });
      ToastAndroid.show(`Login Successful. Welcome ${response.data.fullName} !`, ToastAndroid.LONG);
  ...
  const handleStudentLogin = async () => {
    try {
      ...
      const response = await axios.post('https://attentrackbackend-production.up.railway.app/api/student/login', {
        rollNumber: rollNumber,
        password: password,
      });
      console.log('Login successful:', response.data);
